...
class DatabaseHelper {
  ...
  Future<Map<String, dynamic>?> getNextFlashcard() async {
    ...
    // If all seen, get vocab_id with highest practice count
    final rows = await db.rawQuery("""
      SELECT v.* FROM vocab v
      JOIN flashcard_log f ON v.id = f.vocab_id
      WHERE f.practice_count = (
        SELECT MAX(practice_count) FROM flashcard_log
      )
    """);
